...
from tvcmall_client import get_tvcmall_data, tvc_ready
...
def detect_supplier(url):
    ...
    if "aliexpress." in u:
        return "AliExpress"
    if "tvc-mall." in u or "tvcmall." in u:
        return "TVCMall"
    return None
...
def main():
    ...
    if "TVCMall" in batch_suppliers and not tvc_ready():
        logger.warning(
            "Batch contains TVC Mall rows but TVC_EMAIL/TVC_PASSWORD (or TVC_TOKEN) are not "
            "set - those rows are flagged on the sheet until the secrets exist")
    ...
    for idx, row in batch:
        ...
        if supplier is None:
            continue
        if (supplier == "AliExpress" and not ali_ready()) or (supplier == "TVCMall" and not tvc_ready()):
            ...
            ali_updates = [
                {"range": f"{col_letter(col_map['Status'])}{i}",
                 "values": [["ALIEXPRESS NOT CONNECTED" if supplier == "AliExpress" else "TVCMALL NOT CONNECTED"]]},
            ...
            if "Change Alert" in col_map:
                ali_updates.append({"range": f"{col_letter(col_map['Change Alert'])}{i}",
                                    "values": [[(f"{supplier} is not connected on this store yet - use a "
                                                "different supplier's link for this product, or ask for the "
                                                f"{supplier} keys to be added")]]})
        ...
        try:
            ...
            elif supplier == "TVCMall":
                available, ebay_data = get_tvcmall_data(url)
            else:
        ...
        # Suppliers that quote per-product freight (TVC Mall) override the
        # manual column; eBay/AliExpress leave it as typed (their key is
        # absent). The margin bands then run on cost+shipping as always.
        fetched_shipping = ebay_data.get("shipping_fee") if available else None
        if fetched_shipping is not None:
            shipping_cost = float(fetched_shipping)
        ...
        _FLAG_PREFIXES = ("variant", "choose variant", "unreadable link", "add sku",
                          "invalid sku", "duplicate", "aliexpress", "tvcmall", "protected")
        ...
        if fetched_shipping is not None and "Shipping Cost (£)" in col_map:
